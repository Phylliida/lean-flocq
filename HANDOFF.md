...
| `Calc/Round.lean` | 1404 | `Calc/Round.v` | **Essentially complete.** `cexp_inbetween_float[_loc_Exact]`, `cond_incr`, `inbetween_float_round[_sign]`. All 6 mode families: DN/UP/ZR/N/NE/NA, both unsigned and signed, `inbetween_int_*` and `inbetween_float_*`. `truncate_aux`, `truncate`, `truncate_0`, `truncate_correct_partial[_partial']`/`_correct[_correct']`. Generic correctness: `round_any_correct`, `round_trunc_any_correct[_']`, `round_sign_any_correct`, `round_trunc_sign_any_correct[_']`. **All 30 per-mode aliases** for DN/UP/ZR/NE/NA: plain, trunc, trunc', sign, trunc_sign, trunc_sign'. `truncate_FIX`, `truncate_FIX_correct`. **Deferred:** `generic_format_truncate`/`truncate_correct_format` (need `Zdigits_div_Zpower`). |
