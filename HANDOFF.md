...
**~14310 lines of Lean across 25 files. 0 `sorry`s. All files build clean.**
...
| `Calc/Round.lean` | 1054 | `Calc/Round.v` | **Most of it, including the sign-aware chain.** `cexp_inbetween_float[_loc_Exact]`, `cond_incr`, `inbetween_float_round[_sign]`. All 6 mode families: DN/UP/ZR/N/NE/NA, both unsigned and signed, `inbetween_int_*` and `inbetween_float_*`. `truncate_aux`, `truncate`, `truncate_0`, `truncate_correct_partial[_partial']`/`_correct[_correct']`. Generic correctness: `round_any_correct`, `round_trunc_any_correct[_']`, `round_sign_any_correct`, `round_trunc_sign_any_correct[_']`. Per-mode aliases: `round_DN_correct`, `round_UP_correct`, `round_ZR_correct`, `round_NE_correct`, `round_NA_correct`. `truncate_FIX`, `truncate_FIX_correct`. **Deferred:** more mode-specific aliases (trunc + sign variants), `generic_format_truncate`/`truncate_correct_format` (need `Zdigits_div_Zpower`). |
