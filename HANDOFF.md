...
| `Algorithms/Veltkamp.lean` | 250 | `Pff/Pff2Flocq.v` (Veltkamp section, 323–619) | **In progress.** Landed at FLX: `Veltkamp_C_format` (FLT), `mag_xC_bounds` (between `mag(x)+s` and `mag(x)+s+1`), four `noncomputable def`s for the algorithm constants, `Veltkamp_C_format_FLX`, and three polarity lemmas (`Veltkamp_p_nonneg_FLX`, `Veltkamp_x_le_p_FLX`, `Veltkamp_q_nonpos_FLX`) for `x > 0`. Next: `hxExact_FLX` (Sterbenz on `(p, -q)`), then `Veltkamp_aux_FLX` (case A: `q = -β^s · x` via half-ulp; case B: integer mantissa structural bound). |
